...
| CH10-01 | Reduced activity, reluctance to jump, or stiffness in an aging cat is frequently a sign of joint pain (degenerative joint disease), not simply an inevitable, unaddressable fact of getting older | 10 | S15, S21 | SYNTHESIS | Moderate-High | The teaching claim is attributed to `S15`, the peer-reviewed guideline; `S21`'s specific "90 percent of cats over 12" statistic is used only as an opening hook, not taught as a verified figure — `docs/SOURCES.md`'s own entry for `S21` flags that statistic as unconfirmed against `S15`'s figures. Disclosed per `CLAUDE.md` §19. |
...
| CH10-03 | A senior-care symptom checklist is useful for knowing what to watch for, but which environmental/routine adaptations a household implements should be selected and scaled to that household's real budgets of care, not treated as a fixed checklist every caregiver must fully complete | 10 | S20, S15 | SYNTHESIS | Moderate-High | `docs/SOURCES.md` flags that `S20` alone assumes essentially unlimited owner capacity and must be paired with `S15`'s budgets language; this claim is that disclosed pairing, not a claim either source states alone. Disclosed per `CLAUDE.md` §19. |
