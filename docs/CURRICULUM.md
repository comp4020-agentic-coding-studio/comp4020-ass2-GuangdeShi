...
Chapter 9 carries one open condition inherited from the decision gate: its
pain-recognition evidence gap must be resolved via `CLAUDE.md` §21 before
Chapter 9 is authored in full.
...
| 07 | 7 | III — Keep Her Well | Designing a Healthy Routine | TBD | What does "healthy" look like day to day, not just at the annual checkup? | TBD | Assess body/muscle condition over time and hold an informed preventive-care conversation with a vet | TBD | TBD | S22, S11, S12, S23 | STRUCTURE LOCKED — content pending. Depends on Ch6. |
| 08 | 8 | III — Keep Her Well | The Indoor/Outdoor Decision | TBD | What does safety actually require for *this* cat, in *this* household? | TBD | Evaluate and design a lifestyle/access plan matched to a specific cat's temperament and household's real constraints | TBD | TBD | S24, S1 | STRUCTURE LOCKED — content pending. Depends on Ch1, Ch2. Approved framing: must function as a major welfare/risk/environment/temperament/household trade-off and decision-making chapter, not a facts-only comparison of lifestyles. |
| 09 | 9 | III — Keep Her Well | The Signs Easy to Miss | TBD | What is Momo hiding, and how would I know? | TBD | Recognise subtle pain and dental signs owners commonly miss, and act on them | TBD | TBD | S14 (partial); SOURCE GAP — S13 (pain guideline) access-blocked | STRUCTURE LOCKED — content pending, **conditionally**. Depends on Ch6, Ch7. Approved as a method chapter (like Ch6). Full authoring blocked until the pain-recognition evidence gap is resolved per CLAUDE.md §21 — see "Special requirement" note below. |
...
**Special requirement — Chapter 9.** Before Chapter 9 is drafted in full, the
pain-recognition evidence weakness identified at the decision gate (`S13`
access-blocked; only a secondary summary available) must be resolved using
the Source Supplementation Protocol (`CLAUDE.md` §21) — preferring
peer-reviewed feline pain-recognition research, validated feline
pain-assessment literature, or current authoritative veterinary guidance. If
adequate evidence genuinely cannot be obtained, mark
`CHAPTER 9 EVIDENCE BLOCK — HUMAN REVIEW` and continue only with Chapter 9
work that does not depend on unsupported pain claims.
